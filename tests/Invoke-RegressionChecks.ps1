...
& {
    ...
    $deselectedUpdateKeys = @(); $sawApplyFailure = $false; $stoppedByRoundCap = $false; $outstandingVerificationByVm = @{}; $runEventLog = New-RunEventLogState -Path (Join-Path ([System.IO.Path]::GetTempPath()) ('guestops-events-' + [guid]::NewGuid().ToString('N') + '.jsonl'))
