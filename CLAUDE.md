...
- Default policy first uses structured WUA fields (`MsrcSeverity`, `Type`) when available, then falls back to title/category matching. It preselects critical/important software updates and cumulative/security/critical/rollup + MSRT; it skips driver, preview, feature, and optional updates, and **Microsoft Defender Antivirus security intelligence updates (KB2267602)**. Definitions are skipped for the same reason drivers and previews are: they are republished several times a day with a fresh `RevisionNumber`, so preselecting them would make round N+1's discovery structurally unable to confirm round N and every run would burn `MaxPatchRounds` and exit 1. They stay visible and install normally when the operator ticks them. The match is on the KB id, because the title is localised; other definition families (SCEP, legacy Windows Defender) are **not** covered and are still preselected.
```
