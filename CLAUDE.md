...
Canonical update identity is **`UpdateID|RevisionNumber`**. Discovery writes `updateId`, `revisionNumber`, and `identityKey` for each update when WUA exposes them. The grouped flow selects update groups by `-SelectedUpdateKeys`; `-InstallSelection` is intentionally rejected. The agent still has a legacy `-SelectedUpdateIds` path, but the current orchestrator sends `-SelectedUpdateKeys` during apply.
