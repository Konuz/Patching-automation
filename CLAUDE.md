...
the session. A GUI run answers it in a window through the `ConfirmRescan` provider entry
(`Show-RescanDialog`), so the question never waits unseen in the console behind a dialog.
