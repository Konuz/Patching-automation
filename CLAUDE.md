...
2. **`Start-PatchingGuestOpsGui.ps1`** (optional GUI launcher) — WinForms on PS 5.1, requires an STA host. Collects parameters and credentials in a modal form, saves settings to `%LOCALAPPDATA%\PatchingGuestOps\settings.json` and credentials (DPAPI per key) to `credentials.json`, expands grouped store keys into per-name maps via `Get-GuestCredentialGroups`, then calls the console launcher via `&`. Passes `-PromptProvider`, `-StoredVIServerCredentials` and `-StoredGuestCredentials`; **never** passes `-SelectedUpdateKeys` or `-SkipConfirmation`, as either would end the patch-round loop after the first run. Update group selection, the patch plan approval, the post-install reboot checkpoint and its batch size, the CONTINUE/FINISH round question and the end-of-cycle rescan question are modal dialogs (`SelectUpdateGroups`, `ConfirmPatchPlan`, `ConfirmGuestReboot`, `RebootBatchSize`, `ContinuePatching` and `ConfirmRescan` in the prompt provider); everything else stays in the console. `Show-GuestRebootDialog` lists the same machines and reasons the console prints (`Get-RebootTargetDisplayLines` in the model) and keeps the typed `REBOOT`: the button is **disabled** until the box holds exactly that word, compared with `-ceq` as the console does, so Enter reaches it only once the word is there and Esc or the close button skip the restart. `Show-RebootBatchSizeDialog` follows only when the run supplied no `-RebootBatchSize`, and it has no Cancel - the restart was already approved, so closing it is not an abort. A window that was closed, or any answer below 1 from either surface, becomes **one VM at a time** (`Read-RebootBatchSize` clamps it): the console loop cannot return below 1, but a dialog can, and 0 read as one batch would restart the whole fleet at once. Both questions are asked from inside `Invoke-ApplyAndOptionalReboot`, which therefore takes `-PromptProvider` explicitly rather than reading it off the script scope. `Show-PatchPlanDialog` shows the same listing the console prints, because both render through `Get-PatchPlanDisplayLines`/`Get-PatchPlanSummaryLine` in the model - what the operator approves in a window has to be what the console recorded. Its `AcceptButton` and `CancelButton` are both the refusing button, so Enter, Esc and the window's close button all decline, exactly as an empty answer to the console's `Proceed with this plan? [Y/N]` does: this is the click that starts installing on the fleet. In the update group list a click on a row only selects it: the box changes on a click on the box itself, or Space on the selected row, and `ItemCheck` writes the current value back for anything else. `CheckOnClick = $false` alone is not enough - WinForms then toggles on the second click anywhere on an already-selected row - and reading a row must never change what gets installed on the fleet. `Test-UpdateGroupDialogChecksOnlyOnPurpose` pins both halves in the static gate, since a WinForms list needs an STA host and a desktop the gates cannot assume. The VM list is intentionally not persisted. An **Advanced** checkbox reveals a collapsed block holding the knobs a run normally leaves alone: `-TimeoutMinutes` (180), `-DiscoveryTimeoutMinutes` (30), `-RebootTimeoutMinutes` (30), `-GuestWorkingDirectory`, a `-PatchPlanPath` resume box with a Browse... button, `-PlanOnly` and `-SkipStaticChecks`. The defaults stay the defaults; the block only makes them editable. It sits in the flow rather than over it, so the notice, the buttons and the window height move with it, and the layout is applied once up front as well as on every toggle - the checkbox is remembered, so a window that opens expanded must not paint the group over its own buttons. The three budgets, the guest directory and the block's own state persist to `settings.json`; the resume path, `PlanOnly`, `SearchOnly` and `SkipStaticChecks` never do, because a mode chosen for one run is not a preference and a later launch must not start with a resume or a skipped gate already armed. The GUI launcher clamps the three budgets to `1..35791394` so a typo in a text box is caught in the form rather than at parameter binding after the window has closed. Browse... opens in the operator's output directory (falling back to `<root>\out`) and deliberately offers no "last plan": a run writes one per round (`out\<run>\round-NN\patch-plan.json`), so there is no single last one. A resume combined with `-SearchOnly`, or pointing at a file that is not there, is refused in the form - the launcher refuses both anyway, but minutes later, once the local gates have run and the operator has walked away. The VM field stays required even for a resume: the orchestrator resolves its targets before it reads the saved plan. Every row in the block carries a hover description naming the parameter it sets and what changing it costs, attached to the label as well as the control (`Add-GuiToolTip`) because the label is the wider target and a description only half the hovers find is half a description. `New-GuiToolTip` raises `AutoPopDelay` from the WinForms default of five seconds, which is not long enough to read a sentence, and the component is disposed explicitly - a `ToolTip` is not in `$form.Controls`, so the form does not take it down with itself. This is deliberately not pinned by a gate: a missing tooltip is visible the moment the window opens and cannot change what gets installed.
...
to seconds. The GUI exposes both in its Advanced block and passes whatever is in them, so a GUI
run is no longer pinned to the defaults.
...
Interactive discovery-driven patching has an outer scan-cycle loop around the round loop, and the
two answer different questions. A round continues inside the cycle: it re-discovers only the VMs
that are not green yet and keeps this cycle's decisions, including every group the operator
unticked. A scan cycle starts over. After each cycle's summary `Read-RescanDecision` asks
`Start a fresh full rescan of every VM? [Y/N]`: Y rescans the whole original target list,
green VMs included, with a new update selection and nothing carried over; N or empty input ends
the session. A GUI run answers both questions in a window - the round question through
`ContinuePatching` (`Show-ContinuePatchingDialog`), the rescan through `ConfirmRescan`
(`Show-RescanDialog`) - so neither waits unseen in the console behind a dialog. Only an explicit
`CONTINUE` starts another round: `Read-ContinuePatchingDecision` resolves every other answer to
`FINISH` rather than passing it on, because `Get-PatchRoundDecision` reads an empty decision as
"nobody has answered yet" and would end the run on a reason describing nothing the operator did.
Connections and the guest credential context (including corrected credentials and skip/abort
decisions) live outside that loop. Each
...
The agent and orchestrator are coupled through **`status.json`** (`schemaVersion: phase0b-1`, with a required matching `runId` for orchestrated cycles) and the selected-update command-line arguments.

**Every artifact says which build of the agent wrote it.** `status.json` carries `agentSha256` and `agent.log` opens with the first twelve characters of it. The agent hashes **its own file** rather than being told by the orchestrator, so the value can never describe a different copy than the one that produced the artifact beside it. The path is read at **script scope** and passed into `Get-AgentFileHash`, never read off `$PSCommandPath` inside it: PowerShell sets that per scope from the calling code's source file, and a function defined from a scriptblock built at runtime — which is how the offline fixture loads the agent — has no file of its own, so read inside it came back empty and the build went unrecorded. **No offline gate can hold this wiring**: that same scriptblock leaves `$PSCommandPath` empty wherever it is read, so the fixture asserts the helper against the real agent file and the template's use of `$PSCommandPath` is pinned in the static gate — the same treatment `Get-ComCollectionCount` and `Get-VMGuestReportedHostName` get, and for the same reason. `$null` means the agent could not identify its own file, which is its own answer and never reads as a build. This exists because the stepping stone has no internet and files are copied across by hand: a fleet ran an older agent while the branch had moved on, and the only way to tell the two apart was matching an exception's line number against git history. The agent saves `status.json` eagerly (right after start, and after each stage) so a crash still leaves a trace. The orchestrator parses it defensively via `Get-ObjectPropertyValue` and treats it — not the GuestOps process exit code — as the primary result, because vSphere keeps the exit code for only a short window after the process ends.
...
**`searchResult` is written before anything is counted, and only the update collection may fail the cycle.** The old order counted the updates first and threw on the count, so a guest whose collections PowerShell could not adapt left an artifact with no `ResultCode` in it at all — the one field that says whether the search was even healthy. Now the result record goes down first, then the warnings are harvested, then the updates are counted. Warnings are diagnostic metadata and nothing in the run decides on them, so a warnings collection that cannot be counted, or an entry that refuses to be read, sets `searchResult.warningsUnreadable` and the discovery **stands** — losing a whole VM over them is never the right answer. The update collection is the one count that decides something, so an unreadable one is a real failure, but a named one, raised after the result is already on the record. The same defensive count covers `KBArticleIDs` and the two category readers, which had the identical defect.
