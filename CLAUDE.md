...
Canonical update identity is **`UpdateID|RevisionNumber`**. Discovery writes `updateId`, `revisionNumber`, and `identityKey` for each update when WUA exposes them. The grouped flow selects update groups by `-SelectedUpdateKeys`; `-InstallSelection` is intentionally rejected. During apply, the orchestrator writes the selected `UpdateID|RevisionNumber` keys to `selection.json`, uploads it next to the guest agent, and starts the agent with `-SelectionPath`. The agent still has legacy `-SelectedUpdateIds` and `-SelectedUpdateKeys` CLI paths for compatibility, but the orchestrator no longer depends on comma-joined selected-key payloads.
